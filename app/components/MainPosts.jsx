import React from 'react';

export class MainPosts extends React.Component {
  render () {
		return (
			<div className="full-post">
				<div className="user">
					<div className="user-icon"><a href="/#/user"><img className="avatar" src="http://tu.dd321.com/dianyingpu/tuji/ZhiQingChun2JiaoYuanRePaiXueShengChuZuYa202609.jpg"/></a></div>
					<div className="user-info">	
						<div className="user-name"><a href="/#/user">I_LOVE_JAY</a></div>
						<div className="user-rep"> Rep 289 </div>
					</div>
				</div> 

				<div className="post-body">
					<div className="post-title">Jay Chou's Fan Page</div>
					<div className="post-likes">&hearts; 298</div>
					<hr/>
					<div className="img-container">
						<img src="http://dn-pangpangdog.qbox.me/201501/0131.jpg" alt="jaychou2"/>
					</div>
					<p>Proin rutrum porta leo vel facilisis. Nunc facilisis sit amet nisl hendrerit aliquam. Mauris pharetra interdum porta.</p>
					<p>Nullam nisl velit, iaculis eu libero id, semper posuere neque. Vivamus id quam non eros tempus ultrices. Ut placerat elit metus, eu sagittis justo euismod a. </p>
					<p>Aenean ultricies turpis est, non elementum mauris porttitor vel. Cras pellentesque lobortis odio a ultrices. Vestibulum mattis lacus id mauris dictum, non faucibus libero iaculis. </p>
					<p>Proin accumsan laoreet auctor. Phasellus elementum non mi semper congue. Donec at purus mattis arcu egestas hendrerit. Vivamus aliquet justo ac orci sollicitudin luctus. Etiam ornare dapibus ultrices.</p> 
					<p>Vestibulum sodales lobortis nisl vitae gravida. Mauris eu suscipit leo.</p>
					<div className="img-container">
						<img src="https://yt3.ggpht.com/-2KXvguf9c8E/AAAAAAAAAAI/AAAAAAAAAAA/GKYOlzIEpHc/s900-c-k-no-mo-rj-c0xffffff/photo.jpg" alt="jaychou1"/>
					</div>
					<p>Proin rutrum porta leo vel facilisis. Nunc facilisis sit amet nisl hendrerit aliquam. Mauris pharetra interdum porta.</p>
					<p>Nullam nisl velit, iaculis eu libero id, semper posuere neque. Vivamus id quam non eros tempus ultrices. Ut placerat elit metus, eu sagittis justo euismod a. </p>
					<p>Aenean ultricies turpis est, non elementum mauris porttitor vel. Cras pellentesque lobortis odio a ultrices. Vestibulum mattis lacus id mauris dictum, non faucibus libero iaculis. </p>
					<p>Proin accumsan laoreet auctor. Phasellus elementum non mi semper congue. Donec at purus mattis arcu egestas hendrerit. Vivamus aliquet justo ac orci sollicitudin luctus. Etiam ornare dapibus ultrices.</p> 
					<p>Vestibulum sodales lobortis nisl vitae gravida. Mauris eu suscipit leo.</p>				
				</div>

				
				<div className="post-comments">
					<div className="section-header">&#128488; 35 comments</div>
					<div className="comments">
						<ul>
							<li className="comment">
								<div className="comment-avatar"><img className="avatar" src="https://media.tenor.com/images/ed3cf00e1c90206e943f68d349877690/tenor.gif"/></div>
								<div className="comment-body">	
									<div className="comment-header">
										<a className="user-link">__bibiana_._</a>
										<div className="comment-info"><span className="comment-date">1 day ago</span><span className="comment-likes">&hearts; 29</span></div>
									</div>
									<div className="comment-content">Proin accumsan laoreet auctor.</div>
								</div>
							</li>
							<li className="comment">
								<div className="comment-avatar"><img className="avatar" src="http://www.welovebuzz.com/wp-content/uploads/2014/05/cute-cheeks-o.gif"/></div>
								<div className="comment-body">	
									<div className="comment-header">
										<a className="user-link">gdragoninfaith</a>
										<div className="comment-info"><span className="comment-date">1 day ago</span><span className="comment-likes">&hearts; 2</span></div>
									</div>
									<div className="comment-content">Vestibulum sodales lobortis nisl vitae gravida. Mauris eu suscipit leo.</div>
								</div>
							</li>
							<li className="comment">
								<div className="comment-avatar"><img className="avatar" src="https://m.popkey.co/55bf3b/vzVgk.gif"/></div>
								<div className="comment-body">	
									<div className="comment-header">
										<a className="user-link">Riio</a>
										<div className="comment-info"><span className="comment-date">1 day ago</span><span className="comment-likes">&hearts; 100</span></div>
									</div>
									<div className="comment-content">Vivamus id quam non eros tempus ultrices. Ut placerat elit metus.</div>
								</div>
							</li>
							<li className="comment">
								<div className="comment-avatar"><img className="avatar" src="http://media3.giphy.com/media/hMF1ektHfSPBu/giphy.gif"/></div>
								<div className="comment-body">	
									<div className="comment-header">
										<a className="user-link">Amy</a>
										<div className="comment-info"><span className="comment-date">1 day ago</span><span className="comment-likes">&hearts; 0</span></div>
									</div>
									<div className="comment-content">Nullam nisl velit, iaculis eu libero id, semper posuere neque. Vivamus id quam non eros tempus ultrices. Ut placerat elit metus. Proin accumsan laoreet auctor. Phasellus elementum non mi semper congue.</div>
								</div>
							</li>
							<li className="comment">
								<div className="comment-avatar"><img className="avatar" src="http://tu.dd321.com/dianyingpu/tuji/ZhiQingChun2JiaoYuanRePaiXueShengChuZuYa202609.jpg"/></div>
								<div className="comment-body">	
									<div className="comment-header">
										<a className="user-link">Ed</a>
										<div className="comment-info"><span className="comment-date">1 day ago</span><span className="comment-likes">&hearts; 3</span></div>
									</div>
									<div className="comment-content">Like.</div>
								</div>
							</li>							
						</ul>
					</div>
				</div>

				<div className="featured-pages">
					<div className="section-header">&#9733; Popular Pages</div>

					<a className="featured-page">
						<div className="featured-page-content">
							<div className="page-image">
								<image className="gallery" src="http://www.todayonline.com/sites/default/files/styles/photo_gallery_image_lightbox/public/photos/43_images/photo-128.jpg?itok=gKroWfhI"/>
								<image className="avatar" src="http://imgcache.qq.com/music/photo/album_300/17/300_albumpic_8217_0.jpg"/>
							</div>
							<div className="featured-text">
								<h5>Switch you fav color with Smiley</h5>
								<p>Aenean ultricies turpis est, non elementum mauris porttitor vel. Cras pellentesque lobortis odio a ultrices. Vestibulum mattis lacus id mauris dictum, non faucibus libero iaculis. </p>
							</div>
							<div className="post-info">
								<span className="num-comments">&#128488; 43</span><span className="num-likes">&hearts; 1234</span>
							</div>
						</div>
					</a>

					<a className="featured-page">
						<div className="featured-page-content">
							<div className="page-image">
								<image className="gallery" src="http://www.straitstimes.com/sites/default/files/styles/x_large/public/articles/2015/11/07/jay.jpg?itok=RdYlAr65"/>
								<image className="avatar" src="https://media.tenor.com/images/b3570952aaae135cb2acafe68d03e276/tenor.gif"/>
							</div>
							<div className="featured-text">
								<h5>My Trip to Chinatown</h5>
								<p>Vestibulum mattis lacus id mauris dictum, non faucibus libero iaculis. </p>
							</div>
							<div className="post-info">
								<span className="num-comments">&#128488; 198</span><span className="num-likes">&hearts; 367</span>
							</div>
						</div>
					</a>

					<a className="featured-page">
						<div className="featured-page-content">
							<div className="page-image">
								<image className="gallery" src="http://68.media.tumblr.com/7325898993f698f79a0f09a2e6ce0bfa/tumblr_mhuuzkYFNO1s3pcluo1_400.gif"/>
								<image className="avatar" src="https://365cpop.files.wordpress.com/2013/03/jay-chou-opus-12-shou-yu.jpg"/>
							</div>
							<div className="featured-text">
								<h5>Secret</h5>
								<p>Cras pellentesque lobortis odio a ultrices. Vestibulum mattis lacus id mauris dictum, non faucibus libero iaculis.</p>
							</div>
							<div className="post-info">
								<span className="num-comments">&#128488; 345</span><span className="num-likes">&hearts; 200</span>
							</div>
						</div>
					</a>

					<a className="featured-page">
						<div className="featured-page-content">
							<div className="page-image">
								<image className="gallery" src="http://himg2.huanqiu.com/attachment2010/2015/0119/20150119022840676.jpeg"/>
								<image className="avatar" src="http://orig09.deviantart.net/9665/f/2013/182/c/f/jay_chou___big_ben_mv_by_fierlovesjay-d6bkb9c.gif"/>
							</div>
							<div className="featured-text">
								<h5>Fairytale-like wedding</h5>
								<p>Eon elementum mauris porttitor vel. Cras pellentesque lobortis odio a ultrices. Vestibulum mattis lacus id mauris dictum, non faucibus libero iaculis.</p>
							</div>
							<div className="post-info">
								<span className="num-comments">&#128488; 163</span><span className="num-likes">&hearts; 1034</span>
							</div>
						</div>
					</a>

				</div>
			</div>
		);
	};
};

export default MainPosts;